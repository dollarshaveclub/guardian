...
func newTestConfStore(t *testing.T) (*RedisConfStore, *miniredis.Miniredis) {
	return newTestConfStoreWithDefaults(t, []net.IPNet{}, Limit{}, false)
}

func newTestConfStoreWithDefaults(t *testing.T, defaultWhitelist []net.IPNet, defaultLimit Limit, defaultReportOnly bool) (*RedisConfStore, *miniredis.Miniredis) {
	s, err := miniredis.Run()
	if err != nil {
	...
	return NewRedisConfStore(redis, defaultWhitelist, defaultLimit, defaultReportOnly, TestingLogger), s
}
...
func TestConfStoreReturnsDefaults(t *testing.T) {
	...
	c, s := newTestConfStoreWithDefaults(t, expectedWhitelist, expectedLimit, expectedReportOnly)
	defer s.Close()
...
func TestConfStoreReturnsEmptyWhitelistIfNil(t *testing.T) {
	...
	c, s := newTestConfStoreWithDefaults(t, nil, expectedLimit, expectedReportOnly)
	defer s.Close()
