...
echo "Starting async guardian, sliding window counter"
docker-compose -f e2e/docker-compose-sliding.yml up -d --build --force-recreate > /dev/null 2>&1
[[ $? -gt 0 ]] && exit 1
secs=30 # wait for guardian to finish starting up
while [ $secs -ge 0 ]; do
   echo -ne "Waiting for guardian to start up...$secs\033[0K\r"
...
echo "Stopping sliding window guardian guardian..."
docker-compose -f e2e/docker-compose-sync.yml stop > /dev/null 2>&1
